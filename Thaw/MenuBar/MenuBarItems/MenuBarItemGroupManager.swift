...
@MainActor
final class MenuBarItemGroupManager: ObservableObject {
    ...
    @Published private(set) var groupSet = MenuBarItemGroupSet.empty

    private let defaults: UserDefaults

    /// - Parameter defaults: injectable so tests can use an isolated suite
    ///   instead of the app's real preferences. Mirrors
    ///   `MenuBarSectionController.loadOrder(defaults:)`.
    ...
    private var storageKey: String {
        Defaults.Key.menuBarItemGroups.rawValue
    }
    ...
    private func persist() {
        ...
        do {
            try defaults.set(JSONEncoder().encode(groupSet), forKey: storageKey)
        } catch {
