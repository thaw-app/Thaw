...
/// Hotkeys, Profiles, Integrations) loop continuously — each running its own
/// demo and advancing on its own, wrapping back to Management after
/// Integrations — until the user taps "Get Started" to move on to
/// permissions. The single anchored button (same position and size on every
    ...
    private var pageDots: some View {
        HStack(spacing: 8) {
            ForEach(slides.dropFirst()) { slide in
                ...
                .accessibilityValue(currentSlide == index ? "Current slide" : "Go to slide")
            }
        }
